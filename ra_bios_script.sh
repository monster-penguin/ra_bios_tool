...
# Detect the directory this script lives in (the ra_bios_tool folder)
...
# --- Load config file ---
CONFIG_FILE="$TOOL_DIR/ra_bios_tool.conf"
if [[ -f "$CONFIG_FILE" ]]; then
    source "$CONFIG_FILE"
else
    echo ""
    echo "  WARNING: ra_bios_tool.conf not found in $TOOL_DIR"
...
echo ""
echo "  This tool is not affiliated with RetroARCH in any"
echo "  way whatsoever. Please do not contact RetroARCH"
echo "  for support."
echo ""
echo "======================================================="
echo ""
...
echo "======================================================="
echo ""
echo "  This tool was created for use with RetroARCH"
echo "  version $RA_TESTED_VERSION."
echo ""
echo "  There is no guarantee that this tool will work"
echo "  with previous or subsequent versions of RetroARCH."
echo ""
echo "======================================================="
echo ""
...
echo ""
echo "======================================================="
echo "  Step 1 & 2 - RetroARCH .info Firmware Parser"
echo "======================================================="
echo ""

# --- Step 1: Confirm .info source directory ---
DEFAULT_SOURCE="$RA_MANIFEST_SOURCE"
echo "Confirm location of RetroARCH .info firmware files."
...
mapfile -t INFO_FILES < <(find "$SOURCE_DIR" -name "*.info" 2>/dev/null)
INFO_COUNT=${#INFO_FILES[@]}
echo "Found $INFO_COUNT .info file(s)."

if [[ $INFO_COUNT -eq 0 ]]; then
    echo "No .info files found. Exiting."
    exit 1
fi

# --- Step 2: Confirm combined_manifest.json output location ---
DEFAULT_OUTPUT="$RA_MANIFEST_OUTPUT"
echo ""
...
# --- Step 3: Parse .info files and build combined_manifest.json ---
TMPFILE=$(mktemp /tmp/info_list.XXXXXX)
printf '%s\n' "${INFO_FILES[@]}" > "$TMPFILE"

python3 - "$TMPFILE" "$OUTPUT_FILE" "$RA_BIOS_FOLDER" << 'PYEOF'
import os
import re
import sys
import json
...
bios_folder = sys.argv[3] if len(sys.argv) > 3 else "system"

with open(list_file) as lf:
    info_files = [line.strip() for line in lf if line.strip()]


def parse_info_file(filepath):
    """Parse a RetroARCH .info file (INI-like key = value format)."""
    data = {}
    try:
...
def extract_md5_from_notes(notes):
    """Extract MD5 hashes from the RetroARCH notes field.

    Notes entries are pipe-separated and take the form:
      (!) filename.ext (md5): 32hexchars
    Returns a dict keyed by lowercase filename.
    """
    md5_lookup = {}
    for match in re.finditer(
        r'\(!\)\s*(\S+?)\s*\(md5\):\s*([0-9a-fA-F]{32})',
        notes,
        re.IGNORECASE
    ):
        filename = match.group(1).strip()
        md5hash  = match.group(2).lower()
        md5_lookup[filename.lower()] = md5hash
    return md5_lookup


def clean_description(desc_field, filename):
    """Extract the human-readable portion from a firmware desc field.

    .info desc fields typically look like:
      "filename.ext (Human Readable Description)"
    Returns just the description portion, or the full field if no match.
    """
    # Match: one non-space token, whitespace, then (anything)
    m = re.match(r'^\S+\s+\((.+)\)\s*$', desc_field.strip())
    if m:
        return m.group(1)
    # If the desc field is just the filename with no description, return empty
    if desc_field.strip().lower() == filename.lower():
        return ""
...
for info_path in info_files:
    ...
    firmware_count_str = data.get('firmware_count', '0').strip()
    try:
        firmware_count = int(firmware_count_str)
    except ValueError:
        continue
    ...
    # Prefer systemname; fall back to a cleaned-up core filename
    systemname = data.get('systemname', '').strip()
    if not systemname:
        core_base  = os.path.basename(info_path)
        # Strip trailing _libretro.info or .info
        core_base  = re.sub(r'_libretro\.info$', '', core_base, flags=re.IGNORECASE)
        core_base  = re.sub(r'\.info$', '', core_base, flags=re.IGNORECASE)
        systemname = core_base.replace('_', ' ')

    notes     = data.get('notes', '')
    md5_table = extract_md5_from_notes(notes)

    for i in range(firmware_count):
        fw_path = data.get(f'firmware{i}_path', '').strip()
        fw_desc = data.get(f'firmware{i}_desc', '').strip()
        fw_opt  = data.get(f'firmware{i}_opt',  'false').strip().lower()

        if not fw_path:
            continue

        # Normalise path separators (Windows-style backslash in some files)
        fw_path = fw_path.replace('\\', '/')

        filename = os.path.basename(fw_path)
        subdir   = os.path.dirname(fw_path)   # e.g. "dc" for "dc/dc_boot.bin"

        if not filename:
            continue

        # Build the staging destination path:
        # all RetroARCH firmware lives inside the system (bios_folder) directory,
        # and the firmwareN_path already encodes the subdirectory below that.
        if subdir:
            dest_path = f"{bios_folder}/{subdir}"
        else:
            dest_path = bios_folder

        description = clean_description(fw_desc, filename)

        # firmware0_opt = "true"  → optional → required = "false"
        # firmware0_opt = "false" → required → required = "true"
        # (absent)                → assumed required by RetroARCH convention
        required = "false" if fw_opt == "true" else "true"

        md5hash = md5_table.get(filename.lower(), "")

        # ── Merge duplicate filenames across cores ────────────────
        key = filename.lower()
        if key in seen:
            existing = seen[key]
            if md5hash and md5hash not in existing["md5"]:
                existing["md5"].append(md5hash)
            if systemname and systemname not in existing["system"]:
                existing["system"].append(systemname)
            if dest_path not in existing["paths"]:
                existing["paths"].append(dest_path)
        else:
            seen[key] = {
                "filename":    filename,
                "md5":         [md5hash] if md5hash else [],
                "system":      [systemname] if systemname else [],
...
print(f"Parsed {file_count} .info file(s) with firmware entries.")
print(f"Combined {len(output)} unique firmware entries into combined_manifest.json.")
PYEOF
...
echo "======================================================="
echo ""
echo "  What would you like to do with ra_bios_set.zip?"
echo ""
echo "    E - Use existing ra_bios_set.zip as-is (skip to Step 8)"
echo "    A - Add to existing ra_bios_set.zip from a URL or local directory"
echo "    C - Create a new ra_bios_set.zip"
echo ""
echo -n "  Your choice (E/A/C): "
read -r BIOS_SET_CHOICE

if [[ "$BIOS_SET_CHOICE" =~ ^[Ee]$ ]]; then
    echo ""
    echo "  Using existing ra_bios_set.zip. Proceeding to Step 8."

else
    # --- Step 4 sub: Set zip mode and confirm before proceeding ---
    EXISTING_ZIP="$RA_BIOS_ZIP"
    ZIP_MODE="overwrite"

    if [[ "$BIOS_SET_CHOICE" =~ ^[Aa]$ ]]; then
        # Add mode — append to existing zip without prompting overwrite/append
        ZIP_MODE="append"
        if [[ ! -f "$EXISTING_ZIP" ]]; then
            echo ""
            echo "  NOTE: No existing ra_bios_set.zip found. A new one will be created."
        else
            echo ""
            echo "  Adding to existing ra_bios_set.zip."
            echo "  Files already present with a passing hash will not be replaced."
        fi
    else
        # Create mode — if a zip already exists, ask overwrite or append
        if [[ -f "$EXISTING_ZIP" ]]; then
            echo ""
            echo "  ****WARNING****"
            echo "  An existing ra_bios_set.zip was found."
            echo ""
    ...
    if [[ "$ZIP_MODE" == "overwrite" && -f "$EXISTING_ZIP" ]]; then
        echo ""
        echo "  ****WARNING****"
        echo "  This will overwrite the existing ra_bios_set.zip."
        ...
        if [[ ! "$CONFIRM_ARCHIVE_OP" =~ ^[Yy]$ ]]; then
            echo ""
            echo "  Cancelled. ra_bios_set.zip was not modified."
            echo ""
            exit 0
        fi
    fi
    ...
    STAGE_DIR=$(mktemp -d /tmp/ra_bios_stage.XXXXXX)
    echo ""
    echo "  Temporary staging directory created."

    # Load manifest filenames into a lookup file
    NAMES_FILE=$(mktemp /tmp/ra_bios_names.XXXXXX)
...
PYEOF
    ...
    echo ""
    echo "======================================================="
    echo "  Step 7: Building ra_bios_set.zip"
    echo "======================================================="
    echo ""
    ...
    python3 - "$NAMES_FILE" "$STAGE_DIR" "$OUTPUT_FILE" "$RA_BIOS_ZIP" "${DOWNLOAD_SOURCES[@]}" "${LOCAL_SOURCES[@]}" << 'PYEOF'
...
def existing_entry_passes_hash(filename_lower):
    """Returns True if the file is already in ra_bios_set.zip with a passing hash."""
...
def process_file(filepath, depth=0):
    ...
    if key in target_names:
        ...
        if existing_entry_passes_hash(key):
            # Already in ra_bios_set.zip with a passing hash — do not overwrite
            skipped += 1
...
PYEOF
    ...
    # --- Step 7 cont: Pack staged files into ra_bios_set.zip ---
    DEST_ZIP="$RA_BIOS_ZIP"
...
print(f"  Packed {packed} new file(s) into ra_bios_set.zip.")
if replaced:
...
PYEOF
    ...
    rm -f "$NAMES_FILE"
    echo ""
    echo "  ra_bios_set.zip is ready."
fi

# --- Step 8: Scan ra_bios_set.zip for actual MD5s ---
echo ""
echo "======================================================="
...
DEFAULT_ZIP="$RA_BIOS_ZIP"
echo "Confirm location of ra_bios_set.zip."
echo "Use default: $DEFAULT_ZIP"
echo -n "  Y to proceed, N to enter alternate location: "
read -r CONFIRM_ZIP

if [[ "$CONFIRM_ZIP" =~ ^[Nn]$ ]]; then
    echo -n "Enter alternate path to ra_bios_set.zip: "
    read -r ZIP_FILE
else
...
# --- Step 9: Copy matched files into retroarch/ folder structure ---
echo ""
echo "======================================================="
echo "  Step 9: Build retroarch/ Folder Structure"
echo "======================================================="
echo ""

DEFAULT_RETROARCH_DIR="$RA_STAGING_DIR"
echo "Confirm location for the new retroarch/ output directory."
echo "Use default: $DEFAULT_RETROARCH_DIR"
echo -n "  Y to proceed, N to enter alternate location: "
read -r CONFIRM_RA_DIR

if [[ "$CONFIRM_RA_DIR" =~ ^[Nn]$ ]]; then
    echo -n "Enter alternate path: "
    read -r RETROARCH_DIR
else
    RETROARCH_DIR="$DEFAULT_RETROARCH_DIR"
fi

echo ""
echo "Building folder structure at: $RETROARCH_DIR"

python3 - "$ZIP_FILE" "$OUTPUT_FILE" "$RETROARCH_DIR" << 'PYEOF'
import json, os, sys, zipfile, hashlib

zip_path       = sys.argv[1]
manifest_path  = sys.argv[2]
retroarch_dir  = sys.argv[3]
...
def is_unverified_match(entry):
    """File matched manifest by filename and was found in archive, but RetroARCH
    has no expected MD5 to verify against. Copied to staging by default."""
    return bool(entry.get("actual_md5", "")) and not entry.get("expected_md5", "")

matched_lookup    = {e["filename"].lower(): e for e in manifest if is_match(e)}
unverified_lookup = {e["filename"].lower(): e for e in manifest if is_unverified_match(e)}
    ...
    with zipfile.ZipFile(zip_path, 'r') as zf:
        ...
        for member in members:
            ...
            if key in matched_lookup:
                ...
                paths     = entry.get("paths", ["system"])
                for dest_rel in paths:
                    dest_dir  = os.path.join(retroarch_dir, dest_rel)
                    os.makedirs(dest_dir, exist_ok=True)
                    dest_path = os.path.join(dest_dir, entry["filename"])
                    with open(dest_path, 'wb') as out:
                        out.write(file_data)
                copied += 1
            elif key in unverified_lookup:
                entry     = unverified_lookup[key]
                file_data = zf.read(member.filename)
                paths     = entry.get("paths", ["system"])
                for dest_rel in paths:
                    dest_dir  = os.path.join(retroarch_dir, dest_rel)
...
print(f"  Copied (verified):   {copied} file(s) — hash matched expected MD5.")
if copied_unverified:
    print(f"  Copied (unverified): {copied_unverified} file(s) — filename matched manifest; no expected MD5 in .info files.")
if skipped_mismatch:
...
for root, dirs, files in os.walk(retroarch_dir):
    dirs.sort()
    rel    = os.path.relpath(root, retroarch_dir)
    depth  = 0 if rel == "." else rel.count(os.sep) + 1
    indent = "    " + "  " * depth
    folder = "retroarch/" if rel == "." else os.path.basename(root) + "/"
    print(f"{indent}{folder}  ({len(files)} file(s))")
PYEOF


NO_HASH_COPY_COUNT=$(python3 - "$OUTPUT_FILE" << 'PYEOF'
import json, sys
with open(sys.argv[1]) as f:
    manifest = json.load(f)
print(sum(1 for e in manifest if e.get("actual_md5") and not e.get("expected_md5")))
PYEOF
)
...
if [[ "$HASH_FAIL_COUNT" -gt 0 ]]; then
    ...
    if [[ "$CONFIRM_SAVE_FAILED" =~ ^[Yy]$ ]]; then
        DEFAULT_FAILED_DIR="$RA_FAILED_HASH_DIR"
        echo ""
...
# --- Step 11: Optionally populate live RetroARCH directory ---
echo ""
echo "======================================================="
echo "  Step 11: Populate Live RetroARCH Directory"
echo "======================================================="
echo ""
echo -n "Would you like to populate your existing RetroARCH directory? (Y/N): "
...
if [[ "$CONFIRM_POPULATE" =~ ^[Yy]$ ]]; then
    ...
    echo ""
    echo -n "  Do you wish to proceed with populating the RetroARCH directory? (Y/N): "
    read -r CONFIRM_OVERWRITE

    if [[ "$CONFIRM_OVERWRITE" =~ ^[Yy]$ ]]; then

        DEFAULT_LIVE_DIR="$RA_LIVE_DIR"
        echo ""
        echo "Confirm location of your live RetroARCH config directory."
        echo "Use default: $DEFAULT_LIVE_DIR"
        echo -n "  Y to proceed, N to enter alternate location: "
        read -r CONFIRM_LIVE_DIR

        if [[ "$CONFIRM_LIVE_DIR" =~ ^[Nn]$ ]]; then
            echo -n "Enter alternate path to RetroARCH config directory: "
            read -r LIVE_DIR
        else
        ...
        if [[ ! -d "$LIVE_DIR" ]]; then
            ...
            echo ""
            echo "Copying files from: $RETROARCH_DIR"
            echo "              into: $LIVE_DIR"
            echo ""
            ...
            rsync -av --checksum "$RETROARCH_DIR/" "$LIVE_DIR/"
            RSYNC_EXIT=$?
...
if [[ "$CONFIRM_REPORT" =~ ^[Yy]$ ]]; then
    ...
    DEFAULT_REPORT_DIR="$RA_REPORT_DIR"
    echo ""
    ...
    REPORT_FILE="$REPORT_DIR/retroarch_bios_report.csv"
...
def present_status(entry):
    ...
    # File was found but RetroARCH has no expected MD5 — copied unverified
    if not expected or expected == []:
        return "Yes (unverified — no expected MD5 in .info files)"

    # File was found, check if hash matched
    if isinstance(expected, list):
        matched = actual in expected
    else:
        matched = (actual == expected)
...
def fmt_expected_md5(val):
    """Format expected_md5 for CSV, noting if missing from .info files."""
    if not val or val == "" or val == []:
        return "Missing from RetroARCH .info files"
    if isinstance(val, list):
        return " | ".join(str(v) for v in val if v)
    return str(val)
...
unverified = sum(1 for e in manifest if "unverified" in present_status(e))
...
print(f"  Present (verified): {yes}")
if unverified:
    print(f"  Present (unverified): {unverified}")
...
echo -n "  Delete the temporary retroarch staging folder? (Y/N): "
read -r CONFIRM_DEL_STAGING

echo -n "  Delete ra_bios_set.zip? (Y/N): "
read -r CONFIRM_DEL_ZIP

CONFIRM_SCRUB_ZIP="N"
if [[ ! "$CONFIRM_DEL_ZIP" =~ ^[Yy]$ && "$HASH_FAIL_COUNT" -gt 0 && -f "$ZIP_FILE" ]]; then
    echo ""
    echo "  $HASH_FAIL_COUNT file(s) in ra_bios_set.zip failed hash checks."
    echo "  Would you like to scrub these files from ra_bios_set.zip?"
    ...
    echo ""
    echo -n "  Scrub failed hash check files from ra_bios_set.zip? (Y/N): "
    read -r CONFIRM_SCRUB_ZIP
fi

CONFIRM_SCRUB_NO_HASH="N"
if [[ ! "$CONFIRM_DEL_ZIP" =~ ^[Yy]$ && "$NO_HASH_COPY_COUNT" -gt 0 && -f "$ZIP_FILE" ]]; then
    echo ""
    echo "  $NO_HASH_COPY_COUNT file(s) were copied to staging without hash verification."
    echo "  These files matched the manifest by filename — RetroARCH has no expected MD5"
    echo "  for them. They are most likely valid but cannot be confirmed."
    echo "  Scrubbing them will remove them from ra_bios_set.zip only."
    echo "  Files already copied to staging or your live directory are not affected."
    echo ""
    echo -n "  Scrub unverified files from ra_bios_set.zip? (Y/N): "
    read -r CONFIRM_SCRUB_NO_HASH
fi
...
# Order: staging → zip (+ scrub if applicable) → downloads → manifest (always last)
# The manifest must be deleted last because the scrub operation reads it.
...
if [[ "$CONFIRM_DEL_STAGING" =~ ^[Yy]$ ]]; then
    STAGING_REAL="$(realpath "$RETROARCH_DIR" 2>/dev/null)"
    LIVE_REAL="$(realpath "$RA_LIVE_DIR" 2>/dev/null)"
    HOME_RA_REAL="$(realpath "$HOME/.var/app/org.libretro.RetroArch/config/retroarch" 2>/dev/null)"
    HOME_RA_STANDALONE="$(realpath "$HOME/retroarch" 2>/dev/null)"
    if [[ "$STAGING_REAL" == "$LIVE_REAL" \
       || "$STAGING_REAL" == "$HOME_RA_REAL" \
       || "$STAGING_REAL" == "$HOME_RA_STANDALONE" ]]; then
        echo "  SAFETY CHECK: Staging folder resolves to the live RetroARCH directory."
        echo "  Refusing to delete: $RETROARCH_DIR"
    elif [[ -d "$RETROARCH_DIR" ]]; then
        rm -rf "$RETROARCH_DIR"
        echo "  Deleted: $RETROARCH_DIR"
    else
        echo "  Not found, skipping: $RETROARCH_DIR"
    fi
else
    echo "  Keeping: $RETROARCH_DIR"
fi
...
if [[ "$CONFIRM_DEL_ZIP" =~ ^[Yy]$ ]]; then
    ...
    if [[ "$CONFIRM_SCRUB_ZIP" =~ ^[Yy]$ || "$CONFIRM_SCRUB_NO_HASH" =~ ^[Yy]$ ]]; then
        echo ""
        python3 - "$ZIP_FILE" "$OUTPUT_FILE" "$CONFIRM_SCRUB_ZIP" "$CONFIRM_SCRUB_NO_HASH" << 'PYEOF'
import json, os, sys, zipfile, shutil

zip_path         = sys.argv[1]
manifest_path    = sys.argv[2]
scrub_failed     = sys.argv[3].upper() == "Y"
scrub_unverified = sys.argv[4].upper() == "Y"
...
if scrub_failed:
    for entry in manifest:
        ...
        if isinstance(expected, list):
            ...
            matched = actual == expected
        if not matched:
            scrub_names.add(entry["filename"].lower())

if scrub_unverified:
    for entry in manifest:
        if entry.get("actual_md5") and not entry.get("expected_md5"):
            scrub_names.add(entry["filename"].lower())
...
try:
    with zipfile.ZipFile(zip_path, 'r') as zin, \
        ...
        for member in zin.infolist():
            basename = os.path.basename(member.filename)
            if basename.lower() in scrub_names:
...
PYEOF
        ...
        echo "  Skipping scrub. ra_bios_set.zip was not modified."
    fi
fi
...
# Manifest is always deleted last — the scrub above depends on it being present.
